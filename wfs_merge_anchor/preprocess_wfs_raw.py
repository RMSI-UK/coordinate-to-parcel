...
CLEAN_OUTPUT_SCHEMA = [
    ...
    ("raw_role", object),
    ("clean_area", "float64"),
    ("clean_perimeter", "float64"),
    ("clean_mrr_ratio", "float64"),
    ("clean_hull_gap_ratio", "float64"),
    ("clean_compactness", "float64"),
    ("shape_warning", object),
]
...
def _write_layer(path: Path, layer: str, gdf: gpd.GeoDataFrame) -> None:
    ...
    kwargs = {"geometry_type": "MultiPolygon"} if clean.empty else {}
    clean.to_file(path, layer=layer, driver="GPKG", engine="pyogrio", **kwargs)
...
def _ensure_clean_output_schema(clean: gpd.GeoDataFrame) -> gpd.GeoDataFrame:
    if not clean.empty:
        return clean
    out = clean.copy()
    if out.crs is None:
        out = out.set_crs(DEFAULT_TARGET_CRS)
    for column, dtype in CLEAN_OUTPUT_SCHEMA:
...
def preprocess_wfs_raw(args: argparse.Namespace) -> dict[str, Any]:
    ...
    clean = _ensure_clean_output_schema(clean)
